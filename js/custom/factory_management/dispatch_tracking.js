function loadTable(start, limit) {
    ajaxRequest('../backend/ajax/table_all_dispatched_orders.php', 'POST', {
        start: start,
        limit: limit
    }, function(response) {
...
$(document).ready(function() {
    ...
    $('body').on('click', '.delivered-button', function() {
        openModal('delivered-order-modal', 'static');

        $('#delivered-order-modal').attr('data-id', $(this).attr('data-id'));
    });

    $('body').on('click', '#delivered-order-modal .negative-button', function() {
        $('#delivered-order-modal').attr('data-id', '');

        closeModal('delivered-order-modal');
    });

    $('body').on('click', '#delivered-order-modal .positive-button', function() {
        closeModal('delivered-order-modal');
        openModal('loader-modal', 'static');

        ajaxRequest('../backend/ajax/mark_as_delivered_order.php', 'POST', {
            id: $('#delivered-order-modal').attr('data-id')
        }, function(response) {
