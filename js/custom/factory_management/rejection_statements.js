function loadTable(start, limit) {
    ajaxRequest('../backend/ajax/table_all_rejected_orders.php', 'POST', {
        start: start,
        limit: limit
    }, function(response) {
        ...
        if(response.status === 'Ok' && response.data.length > 0) {
            for(var ctr = 0; ctr < response.data.length; ctr++) {
                $('#orders-table tbody').append('<tr>\
                        <td class="text-center">' + response.data[ctr].tracking_number + '</td>\
                        <td>' + response.data[ctr].products + '</td>\
                        <td>' + response.data[ctr].buyer + '</td>\
                        <td class="text-right">' + response.data[ctr].reason + '</td>\
                    </tr>');
            }
        } else {
            $('#orders-table tbody').append('<tr>\
