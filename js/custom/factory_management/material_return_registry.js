function loadTable(start, limit) {
    ajaxRequest('../backend/ajax/table_all_inventories_return.php', 'POST', {
        start: start,
        limit: limit
    }, function(response) {
        $('#inventories-table tbody').html('');

        if(response.status === 'Ok' && response.data.length > 0) {
            for(var ctr = 0; ctr < response.data.length; ctr++) {
                $('#inventories-table tbody').append('<tr>\
                        <td>' + response.data[ctr].name + '</td>\
                        <td>' + response.data[ctr].description + '</td>\
                        <td>' + response.data[ctr].supplier + '</td>\
                        <td>' + response.data[ctr].reason + '</td>\
                    </tr>');
            }
        } else {
            $('#inventories-table tbody').append('<tr>\
